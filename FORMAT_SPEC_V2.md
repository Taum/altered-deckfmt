# Specification for the binary format (V2)
...
This document describes **version 2** of the binary deck format, matching the implementation in `src/syntax_v2.ts`.
...
```
int(4) version
MUST equal 2. All other values are reserved for future iterations.
...
V2 adds an optional per-group header field which allows compressing `number_in_faction` by storing it as an offset for all but the first card in the group.
...
It is valid to have multiple `SetGroup` elements with the same `set_code`, and decoders MUST support it. Encoders SHOULD split a list of cards where more than 63 cards are from the same set into multiple `SetGroup` elements.

#### Ordering requirement (V2)
...
- Product
- Faction
- Number in faction (with V2 compression)
- Rarity
...
// V2 encoding of number_in_faction:
...
## Appendix A: Main changes from V1

- **Header version**
  - V1: `version` MUST equal 1.
  - V2: `version` MUST equal 2.

- **SetGroup header**
  - V1: `SetGroup` header ends at `refs_count`.
  - V2: `SetGroup` header adds an optional `family_id_bit_length` (4 bits) when `refs_count > 1`.

- **`number_in_faction` encoding**
  - V1: every card stores `number_in_faction` with a set-specific fixed bit length.
  - V2: within each `SetGroup`, the first card stores an **absolute** `number_in_faction` as `int(12)` and establishes `family_id_min`. All subsequent cards store `number_in_faction_offset` using `family_id_bit_length` bits.

- **Ordering requirement**
  - V1: no ordering requirement within a `SetGroup`.
  - V2: encoders MUST ensure the first card in a `SetGroup` has the minimum family id in that group (typically by sorting by `number_in_faction`).

- **Rarity bit length**
  - V1: rarity bit length varies by set (2 bits for older sets, 3 bits for some newer sets).
  - V2: same rule as V1 (as implemented), i.e. still set-dependent.
